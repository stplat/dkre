    ...
    <card>
      ...
      >
        <template v-slot:afterLimit>
          <button class="btn btn--primary mr-1" @click="openModalCreate">Добавить пользователей</button>
          <button class="btn btn--danger" @click="openModalDelete()" :disabled="!userIds.length">Удалить</button>
        </template>
        <template v-slot:checkbox="props">
          <input-checkbox type="blue"
        ...
        <template v-slot:actions="props">
          ...
          <a href="" class="fa fa-edit"
             @click.prevent="openModalDelete(props.row.id)" title="Удалить">
  ...
  computed: {
    table() {
      const headings = {
        ...
        recognitions: "Проверено распознований",
